...
contract FlightSuretyData {
    ...
    mapping(address => bool) private authorizeds;              
    ...
    mapping(bytes32 => bool) private compensationsCredited;               
    ...
    */
    modifier requireIsCallerAuthorized() {
        require(authorizeds[msg.sender] == true || msg.sender == contractOwner, "Caller is not authorized");
        _;
    }
    ...
    */    
    function authorizeCaller(address contractAddress) external requireContractOwner {
        authorizeds[contractAddress] = true;
    }
    ...
    */    
    function deauthorizeContract(address contractAddress) external requireContractOwner {
        delete authorizeds[contractAddress];
    }
    ...
    */   
    function funded(address airline) external view requireIsCallerAuthorized returns(bool) {
    ...
    */
    function creditInsurees(uint256 percentage, address airline, string calldata flight, uint256 scheduledDepartureTime) external requireIsCallerAuthorized {
        bytes32 flightKey = getFlightKey(airline, flight, scheduledDepartureTime);
        require(!compensationsCredited[flightKey], "already been credited for this flight");
    ...
    */
    function() external payable {
        require(msg.data.length == 0, " should be empty");
        require(isAirline(msg.sender), "Sender must be a registered airline");
