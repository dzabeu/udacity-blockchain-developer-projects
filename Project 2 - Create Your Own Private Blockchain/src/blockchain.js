...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
            const t = parseInt(message.split(':')[1]);
            const currTime = parseInt(this._getUTCTimestamp());
            const timeDelay = 5 * 60;
            if (currTime - t > timeDelay) reject(new Error('Bigger than 5 minutes'));
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let blockf = this.chain.find((block) => hash === block.hash);
            if (!blockf) reject(new Error('Block was not found'));
            resolve(blockf);
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            this.chain.forEach(async (block) => {
                ...
                if (dataB && dataB.owner === address) stars.push(dataB);
            });
            resolve(stars);
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i < this.chain.length; i++) {
                ...
                if (currBlock.previousBlockHash !== previousBlock.hash) {
                    err.push({
                        error: 'hash invalid (previous vs current hash)',
                        status:false,
                        block: currBlock
                    });
                }
